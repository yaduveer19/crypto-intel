...
export default function OrderbookPanel({ symbol, exchange }: { symbol: string; exchange?: string }) {
  ...
  const load = useCallback(async () => {
    try {
      setData(await getOrderbook(symbol, exchange))
    } catch {}
    setLoading(false)
  }, [symbol, exchange])
  ...
  const maxDepth = Math.max(
    ...bids.map((b: number[]) => b[1]),
    ...asks.map((a: number[]) => a[1]),
    1e-9,
  )
    ...
    <div className="glass rounded-xl p-4 border border-dark-500">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="text-[10px] text-text-secondary">
          {data?.exchange || '...'} · {data?.mode || ''}
          {mid > 0 && <span className="ml-2 text-text-secondary">spread {fmt(spread)}</span>}
        </div>
      </div>

      {loading && <p className="text-text-secondary text-xs">Loading orderbook...</p>}

      {!loading && asks.length > 0 && (
        <>
          {/* Asks (reversed — best ask on top) */}
